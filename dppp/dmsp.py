...
#         grad = deblur_nb_grad(x, degraded, kernel, noise_stddev) + dmsp_prior_grad(
#             x, denoiser, denoiser_stddev
#         )
...
# GRAD_FN_INPUT_SIGNATURE = (
#     IMAGE_TENSOR_SPEC,
#     IMAGE_TENSOR_SPEC,
#     CHANNEL_KERNEL_TENSOR_SPEC,
#     SCALAR_TENSOR_SPEC,
# )


# @tf.function(input_signature=GRAD_FN_INPUT_SIGNATURE)
# def _deblur_nb_grad_wrap(
#     x: TensorLike, observation: TensorLike, kernel: TensorLike, noise_stddev: TensorLike
# ) -> TensorType:
#     _, data_grad = _compute_deblur_err_grad(
#         x, observation, kernel, mode=ConvolutionModes.WRAP
#     )
#     return (1 / (noise_stddev ** 2)) * data_grad  # type: ignore


# @tf.function(input_signature=GRAD_FN_INPUT_SIGNATURE)
# def _deblur_nb_grad_constant(
#     x: TensorLike, observation: TensorLike, kernel: TensorLike, noise_stddev: TensorLike
# ) -> TensorType:
#     _, data_grad = _compute_deblur_err_grad(
#         x, observation, kernel, mode=ConvolutionModes.CONSTANT
#     )
#     return (1 / (noise_stddev ** 2)) * data_grad  # type: ignore


# ###############################################################################
# # DEBLUR NOISE-ADAPTIVE
# ###############################################################################
...
#     if mode == ConvolutionModes.CONSTANT:
#         deblur_na_grad = _deblur_na_grad_constant
#     elif mode == ConvolutionModes.WRAP:
#         deblur_na_grad = _deblur_na_grad_wrap
#     else:
#         raise ValueError(f"'mode' must be either 'constant' or 'wrap'. Got '{mode}'.")
...
#         # Compute the gradient
#         grad = deblur_na_grad(
#             x, degraded, kernel, 2 * denoiser_stddev ** 2
...
# @tf.function(input_signature=GRAD_FN_INPUT_SIGNATURE)
# def _deblur_na_grad_wrap(
#     x: TensorLike, observation: TensorLike, kernel: TensorLike, sigma2: TensorLike
# ) -> TensorType:
#     data_err, data_grad = _compute_deblur_err_grad(
#         x, observation, kernel, mode=ConvolutionModes.WRAP
#     )
#     lambda_ = _compute_deblur_na_lambda(x, observation, kernel, data_err, sigma2)
#     return lambda_ * data_grad  # type: ignore


# @tf.function(input_signature=GRAD_FN_INPUT_SIGNATURE)
# def _deblur_na_grad_constant(
#     x: TensorLike, observation: TensorLike, kernel: TensorLike, sigma2: TensorLike
# ) -> TensorType:
#     data_err, data_grad = _compute_deblur_err_grad(
#         x, observation, kernel, mode=ConvolutionModes.CONSTANT
...
# # TODO make work for RGB kernels
# def _compute_deblur_na_lambda(
#     x: TensorLike,
...
#     sigma2: TensorLike,
# ) -> TensorType:
#     x_size = img_size(x, tf.float32)
#     obs_size = img_size(observation, tf.float32)
#     sum_data_err2 = tf.math.reduce_sum(data_err ** 2)  # type: ignore
#     # TODO is this correct or do I need the square outside?
...
#     data_grad = convolve(data_err, flip_kernel_2d(kernel), mode=mode)
